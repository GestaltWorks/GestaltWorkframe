...
@pytest.mark.asyncio
async def test_recent_handoff_packets_returns_sanitized_contact_and_intake_packets():
    ...
    try:
        async with session_maker() as session:
            session.add(ContactRecord(
                ...
                data=json.dumps({"company": "Acme", "notes": "Need help\x00 soon."}),
                ip_address="203.0.113.10",
            ))
            session.add(TerminalIntakeRecord(
                terminal_session_id="terminal-session-1",
                selected_mode="automator",
                objective="Get help building or debugging a workflow",
                building="Automation onboarding workflow",
                maturity="Some automation",
                help_needed="Technical answer",
                data=json.dumps({
                    "objective": "Get help building or debugging a workflow",
                    "building": "Automation onboarding workflow",
                    "maturity": "Some automation",
            ...
            packets = await api_main._recent_handoff_packets(session)
    finally:
        async with engine.begin() as conn:
            await conn.run_sync(SQLModel.metadata.drop_all)
        await engine.dispose()

    assert {packet["source"] for packet in packets} == {"contact_form", "terminal_intake"}
    assert any(packet["packet_type"] == "service_inquiry" for packet in packets)
    assert any(packet["packet_type"] == "automator_support" for packet in packets)
...
@pytest.mark.asyncio
async def test_apply_admin_policy_updates_routing_strategy_and_routes():
    route = SimpleNamespace(name="claude-sonnet-4-6")
    router = SimpleNamespace(
        routes=[route],
        set_route_enabled=lambda name, enabled: setattr(route, "enabled", enabled),
        set_routing_strategy=lambda strategy: setattr(route, "strategy", strategy),
        runtime_manager=None,
    )
    budget = SimpleNamespace(
    ...
    services = SimpleNamespace(
        cloud_budget=budget,
        orchestrator=SimpleNamespace(cloud_policy=SimpleNamespace(
            low_cost_enabled=False,
            claude_enabled=False,
            max_cloud_calls_per_turn=0,
            max_cloud_calls_per_session=0,
        )),
        llm_router=router,
    )
    ...
    await api_main._apply_admin_policy(
        services,
        api_main.AdminPolicyPatch(
            cloud_spillover_enabled=True,
            routing_strategy="prefer_cloud_quality",
            claude_enabled=True,
            max_calls_per_turn=1,
            max_calls_per_session=2,
            routes={"claude-sonnet-4-6": False},
        ),
    )

    assert budget.config.enabled is True
    assert services.orchestrator.cloud_policy.claude_enabled is True
    assert services.orchestrator.cloud_policy.max_cloud_calls_per_turn == 1
    assert services.orchestrator.cloud_policy.max_cloud_calls_per_session == 2
    ...
    assert route.strategy == "prefer_cloud_quality"
    budget.init.assert_awaited_once()


@pytest.mark.asyncio
async def test_apply_admin_policy_rejects_unknown_route_names():
    # F3 fix: silently dropping typo'd route names made admins think a toggle
    # had taken effect when it had not. Surface unknown names as 400.
    from fastapi import HTTPException

    route = SimpleNamespace(name="known-route", enabled=True)

    def set_route_enabled(name, enabled):
        route.enabled = enabled

    router = SimpleNamespace(
        routes=[route],
        set_routing_strategy=lambda value: None,
        set_route_enabled=set_route_enabled,
    )
    services = SimpleNamespace(
        cloud_budget=SimpleNamespace(
            config=SimpleNamespace(
                enabled=True, max_calls_per_turn=1, max_calls_per_session=1,
                max_calls_per_day=0, max_calls_per_month=0, max_daily_usd=0.0,
                max_monthly_usd=0.0, max_input_tokens_per_call=0, max_output_tokens_per_call=0,
            ),
            init=AsyncMock(),
        ),
        orchestrator=SimpleNamespace(
            cloud_policy=SimpleNamespace(
                low_cost_enabled=False, claude_enabled=False,
    ...
    with pytest.raises(HTTPException) as excinfo:
        await api_main._apply_admin_policy(
            services,
            api_main.AdminPolicyPatch(routes={"typo-route": False, "known-route": True}),
        )

    assert excinfo.value.status_code == 400
    assert excinfo.value.detail["code"] == "unknown_route_names"
    assert "typo-route" in excinfo.value.detail["routes"]
    # The known route should not have been toggled when validation failed.
...
@pytest.mark.asyncio
async def test_apply_admin_policy_enables_spillover_when_enabling_cloud_tier():
    ...
    await api_main._apply_admin_policy(services, api_main.AdminPolicyPatch(low_cost_enabled=True))

    assert budget.config.enabled is True
    assert services.orchestrator.cloud_policy.low_cost_enabled is True
    budget.init.assert_awaited_once()


@pytest.mark.asyncio
async def test_apply_admin_policy_disables_cloud_tiers_when_spillover_disabled():
    router = SimpleNamespace(routes=[])
    budget = SimpleNamespace(
        config=SimpleNamespace(
            enabled=True,
    ...
    services = SimpleNamespace(
        ...
        orchestrator=SimpleNamespace(cloud_policy=SimpleNamespace(
            ...
            claude_enabled=True,
            max_cloud_calls_per_turn=1,
            max_cloud_calls_per_session=2,
        )),
        llm_router=router,
    )

    await api_main._apply_admin_policy(services, api_main.AdminPolicyPatch(cloud_spillover_enabled=False))

    assert budget.config.enabled is False
    assert services.orchestrator.cloud_policy.low_cost_enabled is False
    assert services.orchestrator.cloud_policy.claude_enabled is False
    budget.init.assert_not_awaited()


@pytest.mark.asyncio
async def test_apply_admin_policy_explicit_spillover_off_wins_over_tier_enable():
    router = SimpleNamespace(routes=[])
    budget = SimpleNamespace(
        config=SimpleNamespace(
            enabled=True,
    ...
    await api_main._apply_admin_policy(
        services,
        api_main.AdminPolicyPatch(cloud_spillover_enabled=False, low_cost_enabled=True),
    )
