...
async def test_retriever_uses_online_fallback_when_local_kb_has_no_context(monkeypatch):
    monkeypatch.setattr("gestaltworkframe.core.retrieval.kb_search", lambda query, limit: "Error searching knowledge base.")
...
async def test_retriever_keeps_local_result_when_fallback_is_unconfigured(monkeypatch):
    monkeypatch.setattr("gestaltworkframe.core.retrieval.kb_search", lambda query, limit: "No relevant information found in the knowledge base.")
...
async def test_retriever_expands_library_workflow_library_queries(monkeypatch):
    ...
    monkeypatch.setattr("gestaltworkframe.core.retrieval.kb_search", fake_search)
...
async def test_retriever_appends_approved_discovery_context(monkeypatch):
    monkeypatch.setattr("gestaltworkframe.core.retrieval.kb_search", lambda query, limit: "Result 1\nSource: docs.md\nContent:\nLibrary docs")
